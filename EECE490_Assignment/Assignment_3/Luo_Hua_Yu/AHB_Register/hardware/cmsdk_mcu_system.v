...
// Check for ifdef ARM_DESIGNSTART_FPGA

`include "cmsdk_mcu_defs.v"
//`define ARM_DESIGNSTART_FPGA
...
`endif
  ...
  //USER DEFINE
  wire              counter0_hsel;  // System control bus interface signals
  wire              counter0_hreadyout;
  wire     [31:0]   counter0_hrdata;
  wire              counter0_hresp;
...
`endif
  ...
  cmsdk_ahb_slave_mux #(
    ...
    u_ahb_slave_mux_sys_bus (
	  ...
	  //User define
	 .HSEL10       (counter0_hsel),         // Input Port 10
    .HREADYOUT10  (counter0_hreadyout),
    .HRESP10      (counter0_hresp),
    .HRDATA10     (counter0_hrdata),
...
cmsdk_ahb_counter counter0(
  // Inputs
 .HCLK(HCLKSYS),      // Clock
 .HRESETn(HRESETn),   // Reset
 .HSEL(counter0_hsel),      // Slave select
 .HTRANS(sys_htrans),    // Transfer type
 .HREADY(sys_hready),    // System ready
 .HADDR(sys_haddr[3:2]),
 .HSIZE(sys_hsize),   // Transfer size
 .HWRITE(sys_hwrite),  // Write control
 .HWDATA(sys_hwdata),  // Write data

  // Outputs
 .HREADYOUT(counter0_hreadyout), // Slave ready
 .HRESP(counter0_hresp),      // Slave response
 .HRDATA(counter0_hrdata)  // Read data output
...
`ifdef ARM_DESIGNSTART_FPGA
  ...
  assign intisr_cm0[15]    = apbsubsys_interrupt[15]   | dma_done | dma_err | ts_interrupt;
`else
